...
- **Documentación Core (`Core-Docs`)**: `D:\Proyectos\biglexj\Core-Docs` (Fuente oficial de documentación, plantillas y estándares compartidos del ecosistema).
...
- **Protocolo de Verificación de Versión en GitHub ("Lanzar actualización") [CRÍTICO]**: Al recibir la indicación del usuario de *"Lanzar actualización"* o iniciar un proceso de publicación:
  ...
  4. **Script de Release [CRÍTICO]**: NUNCA compilar ni publicar binarios manualmente paso a paso. Actualizar `RELEASE_NOTES.md` y `RELEASE_MESSAGE.md`, y luego ejecutar el script centralizado: `d:\Proyectos\biglexj\LyraFlow\build-release.ps1`. Si no existe un script, crear uno basándose en la plantilla de `Core-Docs`. El script se apoya en helpers de `scripts/release/ReleaseTools.ps1` y el empaquetador MSIX en `scripts/packaging/New-MsixPackage.ps1`.
