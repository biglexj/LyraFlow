...
10. **Publicación Oficial de Release en GitHub CLI (`gh release create`) [CRÍTICO]**:
   - Tras hacer `git push origin main --tags`, si el repositorio no cuenta con una acción remota de CI/CD que cree la Release en la web de GitHub, el agente DEBE ejecutar `gh release create vX.Y.Z --title "{Nombre} vX.Y.Z" --notes-file RELEASE_MESSAGE.md` para publicar oficialmente la página de la Release y la entrada en la API de GitHub.
